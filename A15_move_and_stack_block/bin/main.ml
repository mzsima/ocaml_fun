...
let world =  Array.append (Array.create ~len:190 0) (Array.create ~len:10 1)
...
let collision block =
  ...
  let checkCollision target = 
    ...
    value = 1 
    && world.((y' + block.y) * 10 + x' + block.x) = 1 
  in
...
let putWorld block = 
  let projection i b =
    if b = 1 then 
      world.(((i / 4) + block.y - 1) * 10 + block.x + (i mod 4)) <- 1 ;
    ()
  in
...
| "Down" -> speed_up := true
| "Left" -> active_block.x <- active_block.x - 1
| "Right" -> active_block.x <- active_block.x + 1
| _ -> ()
