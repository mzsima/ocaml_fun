    ...
    method draw (ctx : canvasCtx)  = 
      ...
      let draw_circle i j c =
          ...
          let rec draw_inner_circle a b =
            ...
            ctx##stroke ;
            if a < 0.5 then ()
            else draw_inner_circle (a -. 0.32) (b -. 0.2 *. pi)
          in
          draw_inner_circle 1.7  c ;
      in
      for j = 0 to model.count - 1 do
        for i = 0 to model.count - 1 do
          draw_circle i j (float ((j * model.count + i) mod (model.count + 1)) *. 0.2 *. pi)
        done
      done
end
